...
class Blockchain:
    ...
    def create_block(self, proof, previous_hash, privateKey=""):
        ...
        timeBefore = str(datetime.datetime.now()) ### i use timeBefore as salt in rsa sub(from master key) key generation
        if privateKey != "":
            for i in self.transactions:
                print(i)

                senderPrivateKey = self.generator.create_private_key(privateKey, timeBefore) ### creating sub private key from master key and timeBefore
                senderPublicKey = self.generator.create_public_key(privateKey, timeBefore) ### creating sub public key(corresponding to private key) from master key and timeBefore

                ### hash value of sub public key from the sender
                i["sender"]=config.HASH_FUNCTION(senderPublicKey.encode()).hexdigest()

                ### get public key of receiver (sub key from master) to encrypt the content
                receiverPublicKey = requests.post(f'http://127.0.0.1:4999/get-sub-public-key', json={
                    "hashMasterPublic": i["receiver"],
                    "salt": timeBefore
                }).json()["subPublicKey"]

                ### hash value of sub public key from the receiver
    ...
    def add_transaction(self, sender, receiver, amount, flagDefault):
        ...
        if flagDefault == False:
            ### add transaction in negativ value for the sender because it's his money that is sent to other person
            ### this becomes more logical when you think about a wallet
            self.transactions.append({'sender':sender,
                                    'receiver':sender,
                                    'amount': (-1*amount)})
